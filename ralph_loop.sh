...
# OpenCode Ralph Loop with Rate Limiting and Documentation
# Adaptation of the Ralph technique for OpenCode with usage management
...
CLAUDE_ALLOWED_TOOLS="Write,Read,Edit,Grep,Glob,Bash(git *),Bash(npm *),Bash(node *)"  # Optimized tool set for OpenCode development
...
# Session expiration: 12 hours default optimized for OpenCode performance
# Too short = frequent context loss; Too long = stale context causes unpredictable behavior
CLAUDE_SESSION_EXPIRY_HOURS=${CLAUDE_SESSION_EXPIRY_HOURS:-12}  # Optimized for OpenCode: 12 hours for better performance
...
check_claude_version() {
    local version=$($OPENCODE_CMD --version 2>/dev/null | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1)
...
# Build loop context for OpenCode session
...
# - Default expiration: 12 hours (configurable via CLAUDE_SESSION_EXPIRY_HOURS)
# - 12 hours optimized for OpenCode: balances context continuity with fresh performance
# - Sessions auto-expire to ensure OpenCode starts fresh periodically
#
# Returns (stdout):
...
execute_claude_code() {
    ...
    log_status "LOOP" "Executing OpenCode (Call $calls_made/$MAX_CALLS_PER_HOUR)"
    local timeout_seconds=$((CLAUDE_TIMEOUT_MINUTES * 60))
    log_status "INFO" "⏳ Starting OpenCode execution... (timeout: ${CLAUDE_TIMEOUT_MINUTES}m)"
    ...
    if [[ "$use_modern_cli" == "false" ]]; then
        if timeout ${timeout_seconds}s $OPENCODE_CMD < "$PROMPT_FILE" > "$output_file" 2>&1 &
        then
            :  # Continue to wait loop
        else
            log_status "ERROR" "❌ Failed to start OpenCode process"
            return 1
        fi
    fi
...
EOF
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        # Enhanced error detection for OpenCode (3-stage approach)
        # Stage 1: Filter out JSON field patterns like "is_error": false
        # Stage 2: Look for OpenCode-specific errors (API limits, auth, tool failures)
        # Stage 3: General error patterns with improved accuracy

        # Check for OpenCode-specific errors first
        local opencode_errors=false
        if grep -qiE '(rate.limit|api.limit|authentication.failed|unauthorized|invalid.api.key|tool.not.found|permission.denied|timeout|connection.failed)' "$output_file" 2>/dev/null; then
            opencode_errors=true
        fi

        # General error detection (improved patterns)
        local general_errors=false
        if grep -v '"[^"]*error[^"]*":' "$output_file" 2>/dev/null | \
           grep -qE '(^Error:|^ERROR:|^error:|\]: error|Link: error|Error occurred|failed with error|[Ee]xception|Fatal|FATAL|Command failed|Exit code)'; then
            general_errors=true
        fi

        if [[ "$opencode_errors" == "true" || "$general_errors" == "true" ]]; then
            has_errors="true"

            # Debug logging: show what triggered error detection
            if [[ "$VERBOSE_PROGRESS" == "true" ]]; then
                log_status "DEBUG" "Error patterns found:"
                if [[ "$opencode_errors" == "true" ]]; then
                    log_status "DEBUG" "  OpenCode-specific errors detected:"
                    grep -niE '(rate.limit|api.limit|authentication.failed|unauthorized|invalid.api.key|tool.not.found|permission.denied|timeout|connection.failed)' "$output_file" 2>/dev/null | \
                        head -2 | while IFS= read -r line; do
                        log_status "DEBUG" "    $line"
                    done
                fi
                if [[ "$general_errors" == "true" ]]; then
                    log_status "DEBUG" "  General errors detected:"
                    grep -v '"[^"]*error[^"]*":' "$output_file" 2>/dev/null | \
                        grep -nE '(^Error:|^ERROR:|^error:|\]: error|Link: error|Error occurred|failed with error|[Ee]xception|Fatal|FATAL|Command failed|Exit code)' | \
                        head -2 | while IFS= read -r line; do
                        log_status "DEBUG" "    $line"
                    done
                fi
        ...
        if grep -qi "5.*hour.*limit\|limit.*reached.*try.*back\|usage.*limit.*reached" "$output_file"; then
            ...
            log_status "ERROR" "❌ OpenCode execution failed, check: $output_file"
            return 1
        fi
    fi
}
...
main() {
    ...
    log_status "SUCCESS" "🚀 Ralph loop starting with OpenCode"
...
Options:
    ...
    -t, --timeout MIN       Set OpenCode execution timeout in minutes (default: $CLAUDE_TIMEOUT_MINUTES)
